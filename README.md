...
```
~/.copilot/agents/
├── dawbolong.agent.md        ← Copilot 加载的 agent 提示词（必须在根目录）
└── dawbolong/
    └── memory.md             ← 经验库（Agent 自动追加）
```

> ⚠️ Copilot CLI 只扫描 `~/.copilot/agents/*.agent.md`，**不支持子目录**。agent 文件必须放在根目录，子文件夹仅用于存放附属文件。

**大波龙**：vendytam 的私人全能 AI 助手
...
- **记忆机制**：解决非显而易见的问题后自动写入 `~/.copilot/agents/dawbolong/memory.md`
...
bash launch.sh --model=gpt-5.2           # 指定模型
...
- **Agent 文件位置**：`~/.copilot/agents/*.agent.md` 必须在根目录，Copilot CLI **不扫描子目录**；子文件夹可存放 `memory.md` 等附属文件
